...
function _createBooks() {
    ...
    if (!gBooks) gBooks = gDefaultBooks
    storageService.saveToStorage(STORAGE_KEY, gBooks)
}
...
function query(filterBy) {
    ...
    if (filterBy) {
        ...
        minPrice = minPrice || 0
        books = gBooks.filter(book => book.title.includes(name.toLowerCase())
...
function getNextPrevBooks(bookId) {
    const idxById = _getIdxById(bookId)
    
    if (!idxById) return { nextId: gBooks[idxById + 1].id, prevId: gBooks[idxById].id }
    if (idxById === gBooks.length - 1) return { nextId: gBooks[idxById].id, prevId: gBooks[idxById - 1].id }
    return { nextId: gBooks[idxById + 1].id, prevId: gBooks[idxById - 1].id }
}
